...
func init() {
	...
	cmdGroupList.Flags().StringVarP(&criteria.Name, "name", "", "", "Search by group name (supports regex syntax), example: 'foo' will search for the group exactly named 'foo' while '.*foo.*' will search for groups containing 'foo' in their name")
